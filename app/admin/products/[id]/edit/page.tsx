...
export default function EditProductPage() {
    ...
    return (
        <div className="max-w-2xl">
            ...
            <h1 className="text-2xl font-bold mb-8" style={{ color: "#fff" }}>Edit Product #{id}</h1>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                    ...
                    {specs.map((spec, i) => (
                        <div key={i} className="flex gap-3 mb-2 items-center">
                            <input value={spec.label} onChange={(e) => updateSpec(i, "label", e.target.value)}
                                placeholder="Label" className="flex-1 rounded-xl px-3 py-2 text-sm outline-none" style={inputStyle} />
                            <input value={spec.value} onChange={(e) => updateSpec(i, "value", e.target.value)}
                                placeholder="Value" className="flex-1 rounded-xl px-3 py-2 text-sm outline-none" style={inputStyle} />
                ...
                <div className="flex gap-3 pt-2">
                    ...
                    <Link href="/admin/products" className="px-6 py-3 rounded-xl text-sm font-semibold hover:opacity-70" style={{ color: "rgba(255,255,255,0.4)" }}>
                        Cancel
                    </Link>
                </div>
            </form>
